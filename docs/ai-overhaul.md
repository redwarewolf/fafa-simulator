...
### Phase 1 — Shared space model + anticipatory scoring
Status: not started.
- New `PitchControl` utility: coarse grid (reuse `FieldZones`' resolution or a finer ad hoc grid — perf-test both), each cell scored by which team could reach it first factoring in player position **and velocity** (time-to-reach), not just static distance.
- Wire into `CandidatePointScorer`: replace/augment the flat `nearest_opponent_distance` space term with a time-to-reach-based value, so off-ball runs anticipate where space is *opening*.
- Wire into `OnBallUtility.pass_score`: fold distance/angle/pressure/receiver-separation into something closer to a completion-probability scale (xPass-model-inspired), replacing the current fixed-constant lane/openness penalties.
- Validate: pitch-control heatmap overlay tracks real open space; `2v1_break`/through-ball scripted scenarios; full-match eye test.
